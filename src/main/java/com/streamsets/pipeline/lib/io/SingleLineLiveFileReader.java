...
public class SingleLineLiveFileReader implements LiveFileReader {
    ...
    public boolean hasNext() throws IOException {
        ...
        if (LOG.isTraceEnabled()) {
            LOG.trace("Dongbin## currentFile '{}' at channel position '{}', and buffer position '{}' , isEof ? '{}'", currentFile, channel.position(), buffer.position(), isEof());
        }
    ...
    private boolean isEof() throws IOException {
        try {
            if (LOG.isTraceEnabled()) {
                LOG.trace("Dongbin##start isEof(): originalFile? '{}', currentFile '{}' , rolled '{}'", originalFile, currentFile, rolled);
            }
            if (!rolled) {
                /**这里删除或者挪走正在采集的文件后，currentFile在前面经过refresh之后就被变成了null，但是originalFile不为空，因此，下面的循环进不去*/
                if (originalFile.equals(currentFile) && System.currentTimeMillis() - lastLiveFileRefresh > REFRESH_INTERVAL) {
                    currentFile = originalFile.refresh();

                    if (LOG.isTraceEnabled()) {
                        LOG.trace("Dongbin##in isEof() after refresh : originalFile? '{}', currentFile '{}'", originalFile, currentFile);
                    }
                    /**日志被删除了或者被挪走了*/
                    if (currentFile == null) {
                        return true;
            ...
            if (LOG.isTraceEnabled()) {
                LOG.trace("Dongbin##end isEof() after refresh : rolled '{}' , channel.position()? '{}', channel.size '{}'", rolled, channel.position(), channel.size());
            }
