...
public class LiveDirectoryScanner {
    ...
    /**current如果为空，是不会报“is not acceptable for live file”错误的*/
    private LiveFile scanInternal(LiveFile current) throws IOException {
        Utils.checkArgument(
                current == null || rollMode.isCurrentAcceptable(current.getPath().getFileName().toString()),


                Utils.formatL("Current file '{}' is not acceptable for live file '{}' using '{}'", current, liveFileName, rollMode));
